...
async function checkSplashScreenImages(projectDir) {
  const {
    exp
  } = await ProjectUtils.readConfigJsonAsync(projectDir); // return before SDK33

  if ((0, _ExponentTools.parseSdkMajorVersion)(exp.sdkVersion) < 33) {
    return;
  }

  const splashScreenMode = _lodash.default.get(exp, 'android.splash.resizeMode') || _lodash.default.get(exp, 'splash.resizeMode'); // only mode `native` is handled by this check
