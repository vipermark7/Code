# -*- encoding: utf-8 -*-
# stub: google-cloud-env 1.0.5 ruby lib

Gem::Specification.new do |s|
  s.name = "google-cloud-env"
  s.version = "1.0.5"

  s.required_rubygems_version = Gem::Requirement.new(">= 0") if s.respond_to? :required_rubygems_version=
  s.require_paths = ["lib"]
  s.authors = ["Daniel Azuma"]
  s.date = "2018-09-21"
  s.description = "google-cloud-env provides information on the Google Cloud Platform hosting environment. Applications can use this library to determine hosting context information such as the project ID, whether App Engine is running, what tags are set on the VM instance, and much more."
  s.email = ["dazuma@google.com"]
  s.homepage = "https://github.com/googleapis/google-cloud-ruby/tree/master/google-cloud-env"
  s.licenses = ["Apache-2.0"]
  s.required_ruby_version = Gem::Requirement.new(">= 2.0.0")
  s.rubygems_version = "2.4.5"
  s.summary = "Google Cloud Platform hosting environment information."

  s.installed_by_version = "2.4.5" if s.respond_to? :installed_by_version

  if s.respond_to? :specification_version then
    s.specification_version = 4

    if Gem::Version.new(Gem::VERSION) >= Gem::Version.new('1.2.0') then
      s.add_runtime_dependency(%q<faraday>, ["~> 0.11"])
      s.add_development_dependency(%q<minitest>, ["~> 5.10"])
      s.add_development_dependency(%q<minitest-autotest>, ["~> 1.0"])
      s.add_development_dependency(%q<minitest-focus>, ["~> 1.1"])
      s.add_development_dependency(%q<minitest-rg>, ["~> 5.2"])
      s.add_development_dependency(%q<autotest-suffix>, ["~> 1.1"])
      s.add_development_dependency(%q<redcarpet>, ["~> 3.0"])
      s.add_development_dependency(%q<rubocop>, ["~> 0.50.0"])
      s.add_development_dependency(%q<simplecov>, ["~> 0.9"])
      s.add_development_dependency(%q<yard>, ["~> 0.9"])
      s.add_development_dependency(%q<yard-doctest>, ["~> 0.1.13"])
    else
      s.add_dependency(%q<faraday>, ["~> 0.11"])
      s.add_dependency(%q<minitest>, ["~> 5.10"])
      s.add_dependency(%q<minitest-autotest>, ["~> 1.0"])
      s.add_dependency(%q<minitest-focus>, ["~> 1.1"])
      s.add_dependency(%q<minitest-rg>, ["~> 5.2"])
      s.add_dependency(%q<autotest-suffix>, ["~> 1.1"])
      s.add_dependency(%q<redcarpet>, ["~> 3.0"])
      s.add_dependency(%q<rubocop>, ["~> 0.50.0"])
      s.add_dependency(%q<simplecov>, ["~> 0.9"])
      s.add_dependency(%q<yard>, ["~> 0.9"])
      s.add_dependency(%q<yard-doctest>, ["~> 0.1.13"])
    end
  else
    s.add_dependency(%q<faraday>, ["~> 0.11"])
    s.add_dependency(%q<minitest>, ["~> 5.10"])
    s.add_dependency(%q<minitest-autotest>, ["~> 1.0"])
    s.add_dependency(%q<minitest-focus>, ["~> 1.1"])
    s.add_dependency(%q<minitest-rg>, ["~> 5.2"])
    s.add_dependency(%q<autotest-suffix>, ["~> 1.1"])
    s.add_dependency(%q<redcarpet>, ["~> 3.0"])
    s.add_dependency(%q<rubocop>, ["~> 0.50.0"])
    s.add_dependency(%q<simplecov>, ["~> 0.9"])
    s.add_dependency(%q<yard>, ["~> 0.9"])
    s.add_dependency(%q<yard-doctest>, ["~> 0.1.13"])
  end
end
